...
class SolverPanel extends JPanel {
    ...
    private final JRadioButton bfBtn    = new JRadioButton("Brute Force");

    private final ButtonGroup  hGroup = new ButtonGroup();
    private final JRadioButton h2Btn  = new JRadioButton("H1 – Manhattan via checkpoint");
    private final JRadioButton h3Btn  = new JRadioButton("H2 – Euclidean via checkpoint");
    ...
    // callback(algorithm, heuristic): algorithm 0=UCS,1=A*,2=GBFS; heuristic 1/2/3
    ...
    SolverPanel() {
        ...
        // --- Heuristic section ---
        hGroup.add(h2Btn); hGroup.add(h3Btn);
        h2Btn.setSelected(true);
        styleRadio(h2Btn); styleRadio(h3Btn);
    ...
    void setSolving(boolean solving) {
        ...
        h2Btn.setEnabled(!solving); h3Btn.setEnabled(!solving);
